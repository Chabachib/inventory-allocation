...
class InventoryAllocationSystem:
    ...
    VERSION 3: Incorporates dynamic column mapping for generalized use.
    """
    
    def __init__(self, forecast_file, stock_file, forecast_mapping, stock_mapping):
    ...
    def load_and_clean_data(self):
        ...
        try:
            ...
            # --- DYNAMIC MAPPING ---
            # Create standardized forecast_df
        ...
        # --- DATA CLEANING (on standardized columns) ---
        # Clean Forecast Data
        forecast_df['forecast_quantity'] = forecast_df['forecast_quantity'].astype(str).str.strip().replace(' - ', '0')
        ...
        # Clean Stock Data
        stock_df['expiry_date'] = pd.to_datetime(stock_df['expiry_date'], errors='coerce') # More flexible date parsing
    ...
    def run_analysis(self):
        ...
        Stock is treated as a shared pool at the branch level.
        (No changes needed in this method as it uses the standardized self.forecast_df and self.stock_df)
        """
        # 1. Aggregate destination branch stock
        destination_stock = self.stock_df[self.stock_df['branch_id'] != 1000]
        ...
        # 2. Aggregate forecast at the product-branch-channel level
        agg_forecast = self.forecast_df.groupby(['product_id', 'branch_id', 'channel_id'])['forecast_quantity'].sum().reset_index()
        
        # 3. Merge channel-level forecast with branch-level stock
        ...
        # 4. Calculate total branch forecast
    ...
    def run_allocation(self, priority_channels=[], priority_branches=[]):
        """
        Runs the prioritized FEFO allocation algorithm.
        ...
        cw_stock_sorted = self.stock_df[self.stock_df['branch_id'] == 1000].copy().sort_values(by=['product_id', 'expiry_date'])
        ...
        for _, demand_row in prioritized_demand_df.iterrows():
            ...
            available_batches = cw_stock_sorted[cw_stock_sorted['product_id'] == prod_id]
            ...
            for batch_idx, batch_row in available_batches.iterrows():
                ...
                allocation_plan.append({'product_id': prod_id, 'from_branch': 1000, 'to_branch': branch_id, 'channel_id': channel_id, 'quantity_allocated': take, 'expiry_date': batch_row['expiry_date']})
